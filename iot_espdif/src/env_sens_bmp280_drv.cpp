...
EnvSensBMP280Drv::EnvSensBMP280Drv(bmp280_config_t *bmp280_config, i2c_master_bus_handle_t bus_handle, II2cMasterWrapper &ii2cmasterwrapper) 
...
return_code EnvSensBMP280Drv::init()
{
    ...
    if (m_ii2cmasterwrapper.transmit_receive(m_dev_handle, reg_addr, 1, reg_calib_data, 24, -1) != ESP_OK)
    {
        m_state = error;
    }
...
void EnvSensBMP280Drv::process()
{
    ...
    if (((m_state == busy) && (is_sensor_measuring == false)) || (m_state == ok))
    {
        ...
        if (m_ii2cmasterwrapper.transmit_receive(m_dev_handle, addr, 1, buffer, 6, -1) != ESP_OK)
        {
            m_state = error;
            return;
        }
