...
return_code EnvSensBMP280Drv::init()
{
    ...
    if (i2c_master_transmit_receive(m_dev_handle, reg_addr, 1, reg_calib_data, 24, -1) != ESP_OK)
    {
        m_state = error;
    }
...
return_code EnvSensBMP280Drv::readHumidity(uint32_t &humidity)
{
...
void EnvSensBMP280Drv::process()
{
    ...
    if (((m_state == busy) && (is_sensor_measuring == false)) || (m_state == ok))
    {
        ...
        if (i2c_master_transmit_receive(m_dev_handle, addr, 1, buffer, 6, -1) != ESP_OK)
        {
            m_state = error;
            return;
        }
